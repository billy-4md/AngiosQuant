...
def save_image_function(image, image_name):
    if save_images:
        imageio.volwrite(os.path.join("D:\\Users\\MFE\\Xavier\\PHALLO", image_name), image)
...
def launch_segmentation_process():
    ...
    images = ["image1"]
    diam_value_list = [120, 150, 175, 200]
    ...
    tag_label = "488"
    image = "image1"
    tag_img = isolate_and_normalize_channel(tag_label, image)
    tag_img_cropped = tag_img[up_slice_to_remove1:-down_slice_to_remove1, cropped_value:-cropped_value, cropped_value:-cropped_value]
    save_image_function(tag_img_cropped, f'PHALO_original.tif')
    ...
    for diam_value in diam_value_list:
        ...
        # #3D
        # masks, flows, styles, diams = model.eval(tag_img_cropped, diameter=diam_value, channels=[0,0], do_3D =True)
        # masks = masks.astype(np.uint16)
        # save_image_function(masks, f"CellPose_pred_3D_nuclei_diam{diam_value}.tif")

        #2D        
        cellpose_pred, data = do_segmentation(tag_img_cropped, 100000, 0, diam_value)
        df = pd.DataFrame(data)
        labels = reassign_labels(cellpose_pred, df)
        save_image_function(labels, f'CellPose_pred_2D_cyto3_diam{diam_value}.tif')
